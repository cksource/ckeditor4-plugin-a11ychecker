...
( function() {
	...
	CKEDITOR.ui.balloonPanel.prototype = {
		...
		attach: ( function() {
			...
			return function( element, focusElement ) {
				...
				if ( isInline ) {
					...
					allowedRect = this._getWindowRect( winGlobal );
				}
				this._adjustElementRect( elementRect, isInline ? this._getWindowRect( winGlobal ) : editorRect );

				// The area of the panel.
				var panelArea = panelWidth * panelHeight,
					alignments = this.getAlignments( elementRect, panelWidth, panelHeight ),
		...
		getAlignments: function( elementRect, panelWidth, panelHeight ) {
		...
		_adjustElementRect: function( elementRect, editorRect ) {
			var triWidth = this.triangleWidth,
				triHeight = this.triangleHeight;

			elementRect.left = numberInRange( editorRect.left, editorRect.right - triWidth - 1, elementRect.left );
			elementRect.right = numberInRange( editorRect.left, editorRect.right - triWidth, elementRect.right );
			elementRect.top = numberInRange( editorRect.top, editorRect.bottom - triHeight - 1, elementRect.top );
			elementRect.bottom = numberInRange( editorRect.top, editorRect.bottom - triHeight, elementRect.bottom );
		},

		_getWindowRect: function( window ) {
			var pos = window.getScrollPosition(),
				viewSize = window.getViewPaneSize();

			return {
				top: pos.y,
				bottom: pos.y + viewSize.height + this.triangleHeight,
				left: pos.x,
				right: pos.x + viewSize.width + this.triangleWidth
			};
		}
	};
